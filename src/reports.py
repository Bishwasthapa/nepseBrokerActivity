...
function loadBroker(){var id=val('brok-id');if(!id)return;api('broker/'+id+'?top='+(val('brok-top')||5)+'&sessions='+(val('brok-sess')||66)).then(function(data){if(!data)return;q('brok-h').textContent='Broker '+id+' \u2014 top holdings';q('brok-out').innerHTML=renderTable(data.holdings,HOLD_COLS);bindClicks(q('brok-out'));loadLazyVerdicts('brok-out');});}
var MOM_COLS=[{key:'symbol',label:'Symbol',type:'sym',desc:'NEPSE ticker symbol (click to inspect)'},{key:'verdict',label:'Verdict',type:'verdict_lazy',desc:'Long-only positional verdict (hover for breakdown)'},{key:'avg_rank_base',label:'Avg Rank (Base)',type:'num',desc:'Average daily turnover rank over baseline window (1 = most traded)'},{key:'avg_rank_short',label:'Avg Rank (Short)',type:'num',desc:'Average daily turnover rank over recent window (1 = most traded)'},{key:'rank_drift',label:'Rank Drift',type:'num',desc:'Avg Base Rank − Avg Recent Rank (positive = climbed the turnover board)'},{key:'turnover_ratio',label:'Turnover Ratio',type:'num',desc:'Recent average daily turnover ÷ baseline average (e.g. 2.0 = twice as liquid recently)'},{key:'close',label:'Close',type:'num',desc:'Latest session closing price in NRS'},{key:'price_change_pct_window',label:'Wnd Change %',type:'pct',desc:'Close price change % across the recent window'},{key:'top_accumulator',label:'Top Accum',type:'broker',desc:'Broker with largest net buy in recent window (click to inspect)'},{key:'top_distributor',label:'Top Distrib',type:'broker',desc:'Broker with largest net sell in recent window (click to inspect)'}];
